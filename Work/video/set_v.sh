...
# Tag removal patterns file
TAG_REMOVAL_FILE="${CONFIG_DIR}/remove_tags.txt"
...
# Function to load tag removal patterns from file
load_tag_patterns() {
    local -a patterns=()
    
    # Default patterns (always included)
    local default_patterns=(
        "BluRay" "BRRip" "BDRip" "HDRip" "WEBRip" "WEB-DL" "DVDRip" "HDTV" "SDTV"
        "1080p" "720p" "480p" "2160p" "4K" "UHD" "10bit" "8bit"
        "x264" "x265" "h264" "h265" "HEVC" "AVC" "AV1" "XviD" "DivX"
        "AAC" "AC3" "DTS" "DTS-HD" "TrueHD" "FLAC" "MP3" "Atmos" "LPCM" "DD5.1" "DD2.0"
        "5\\.1" "7\\.1" "2\\.0" "6\\.1" "10\\.0"
        "EXTENDED" "UNRATED" "REMASTERED" "DIRECTORS[._-]*CUT" "DC" "FINAL"
        "PROPER" "REPACK" "INTERNAL" "LIMITED" "COMPLETE" "RETAIL"
        "DUBBED" "SUBBED" "SUB" "DUB" "REMUX"
        "YIFY" "YTS" "YTS\\.MX" "RARBG" "TGx" "EZTV" "EVO" "BONE"
        "www\\.[a-z0-9]+\\.[a-z]+" "[0-9]{3,4}MB" "MOVIES" "FILM"
    )
    
    patterns=("${default_patterns[@]}")
    
    # Load additional patterns from file if exists
    if [ -f "$TAG_REMOVAL_FILE" ]; then
        ...
        done < "$TAG_REMOVAL_FILE"
    fi
    
    # Return patterns as array (use global variable)
    TAG_PATTERNS=("${patterns[@]}")
}

# Function to scan filenames and learn new patterns
learn_tag_patterns() {
    local source_dir="$1"
    local -A found_tags
    local min_occurrences=2  # Tag must appear at least twice to be learned
    
    echo "Analyzing filenames to detect common tags..."
    ...
    while IFS= read -r -d '' file; do
        ...
        # Extract potential tags (uppercase words, quality indicators, etc.)
        # Look for: WORD in CAPS, resolution patterns, codec patterns, etc.
        while read -r potential_tag; do
            if [ -n "$potential_tag" ]; then
                ((found_tags["$potential_tag"]++))
            fi
        done < <(echo "$base_name" | grep -oE '([A-Z]{2,}[0-9]*|[0-9]{3,4}[pP]|[0-9]\.[0-9]|[xXhH][0-9]{3,4}|www\.[a-z0-9\.]+)' | sort -u)
    done < <(find "$source_dir" -type f \( -iname "*.avi" -o -iname "*.mkv" -o -iname "*.mp4" -o -iname "*.mov" -o -iname "*.wmv" -o -iname "*.flv" -o -iname "*.webm" -o -iname "*.m4v" \) -print0 2>/dev/null)
    
    # Filter and save new patterns
    local new_patterns=()
    for tag in "${!found_tags[@]}"; do
        local count=${found_tags[$tag]}
        if [ "$count" -ge "$min_occurrences" ]; then
            # Check if not already in our pattern list
            local already_exists=false
            for existing in "${TAG_PATTERNS[@]}"; do
                if [[ "$tag" =~ ^${existing}$ ]] || [[ "$existing" =~ ^${tag}$ ]]; then
                    already_exists=true
                    break
                fi
            done
            ...
            if [ "$already_exists" = false ]; then
                new_patterns+=("$tag")
                echo "  Detected new tag: $tag (found $count times)"
            fi
        fi
    done
    
    # Save new patterns to file
    if [ ${#new_patterns[@]} -gt 0 ]; then
        mkdir -p "$CONFIG_DIR" 2>/dev/null
        echo "# Auto-detected tag patterns - $(date)" >> "$TAG_REMOVAL_FILE"
        for pattern in "${new_patterns[@]}"; do
            echo "$pattern" >> "$TAG_REMOVAL_FILE"
        done
        echo "Saved ${#new_patterns[@]} new tag pattern(s) to $TAG_REMOVAL_FILE"
        
        # Reload patterns
        load_tag_patterns
    else
        echo "No new tag patterns detected"
    fi
}

# Function to clean filename using tag patterns
clean_filename_with_patterns() {
    local filename="$1"
    local cleaned="$filename"
    
    # Apply each pattern
    for pattern in "${TAG_PATTERNS[@]}"; do
        # Remove pattern with surrounding separators (case insensitive)
        cleaned=$(echo "$cleaned" | sed -E "s/[._-]*${pattern}[._-]*//gi")
    done
    
    # Clean up multiple/trailing separators
    cleaned=$(echo "$cleaned" | sed -E 's/[._-]{2,}/./g; s/[._-]+$//; s/^[._-]+//')
    
    echo "$cleaned"
}

# Load tag patterns on startup
load_tag_patterns
...
extract_movie_info() {
    ...
    # Use pattern-based cleaning (new intelligent method)
    base_name=$(clean_filename_with_patterns "$base_name")
...
find_subtitle_files() {
    ...
    # Track found subtitles to avoid duplicates (using size+lang as key for better deduplication)
    local -A found_subtitles
    local -A subtitle_signatures
    
    # Get the directory containing the video file if provided
    local video_dir=""
    local video_parent_dir=""
    if [ -n "$original_video_file" ] && [ -f "$original_video_file" ]; then
        video_dir=$(dirname "$original_video_file")
    elif [ -n "$original_video_file" ]; then
        # Video file already moved, try to find its original directory
        local video_basename=$(basename "$original_video_file")
        video_basename="${video_basename%.*}"  # Remove extension
        
        # Clean the basename - remove year, language tags, etc.
        local clean_basename=$(echo "$video_basename" | sed -E 's/\.[0-9]{4}.*//; s/_[a-z]{3}$//; s/\([0-9]{4}\).*//; s/[._-]*$//')
        
        # Search for directory containing video or similar name
        while IFS= read -r -d '' found_dir; do
            video_parent_dir="$found_dir"
            break
        done < <(find "$source_dir" -type d \( -iname "*${video_basename}*" -o -iname "*${clean_basename}*" \) -print0 2>/dev/null | head -1)
        
        # If not found by basename, try searching by movie title
        if [ -z "$video_parent_dir" ]; then
            local title_search=$(echo "$movie_title" | sed 's/ /*/g')
            while IFS= read -r -d '' found_dir; do
                video_parent_dir="$found_dir"
                break
            done < <(find "$source_dir" -type d -iname "*${title_search}*" -print0 2>/dev/null | head -1)
        fi
    fi
    
    # Strategy 1: Find subtitles recursively in the parent directory (including Sub/Subs/Subtitle/Subtitles folders)
    if [ -n "$video_parent_dir" ] && [ -d "$video_parent_dir" ]; then
        while IFS= read -r -d '' sub_file; do
            if [ -f "$sub_file" ]; then
                local sub_path=$(realpath "$sub_file" 2>/dev/null || echo "$sub_file")
                local sub_size=$(stat -f%z "$sub_file" 2>/dev/null || stat -c%s "$sub_file" 2>/dev/null || echo "0")
                local sub_base=$(basename "$sub_file")
                local sub_lang="unknown"
                
                # Extract language code from subtitle filename
                if [[ "$sub_base" =~ [._-](${LANG_PATTERN})[._-]? ]]; then
                    sub_lang="${BASH_REMATCH[1]}"
                fi
                
                local signature="${sub_size}_${sub_lang}"
                if [ -z "${found_subtitles[$sub_path]}" ] && [ -z "${subtitle_signatures[$signature]}" ]; then
                    found_subtitles[$sub_path]=1
                    subtitle_signatures[$signature]=1
                    echo "$sub_file"
                fi
            fi
        done < <(find "$video_parent_dir" -type f \( -iname "*.srt" -o -iname "*.sub" -o -iname "*.ass" -o -iname "*.ssa" -o -iname "*.vtt" -o -iname "*.smi" \) -print0 2>/dev/null)
    fi
    
    # Strategy 2: Find subtitles in the same directory as the video file (before it was moved)
    ...
    # Strategy 3: If still no results, search in common subtitle folder names
    if [ ${#found_subtitles[@]} -eq 0 ] && [ -n "$video_parent_dir" ]; then
        for subdir in "Sub" "Subs" "Subtitle" "Subtitles" "subs" "subtitle" "subtitles"; do
            local sub_folder="${video_parent_dir}/${subdir}"
            if [ -d "$sub_folder" ]; then
                ...
                done < <(find "$sub_folder" -type f \( -iname "*.srt" -o -iname "*.sub" -o -iname "*.ass" -o -iname "*.ssa" -o -iname "*.vtt" -o -iname "*.smi" \) -print0 2>/dev/null)
            fi
        done
    fi
    
    # Strategy 4: Fuzzy matching based on normalized names (last resort)
...
# Function to verify if NFO file matches movie title
verify_nfo_matches_movie() {
    local nfo_file="$1"
    local expected_title="$2"
    
    # Check if file contains "for KODI"
    if ! grep -qi "for KODI" "$nfo_file" 2>/dev/null; then
        return 1
    fi
    
    # Extract title from NFO file (XML format)
    local nfo_title=""
    nfo_title=$(grep -oP '<title>\K[^<]+' "$nfo_file" 2>/dev/null | head -1)
    ...
    local norm_expected=$(normalize_string "$expected_title")
    local norm_nfo=$(normalize_string "$nfo_title")
    
    # Check if titles match
    if [ "$norm_expected" = "$norm_nfo" ]; then
        return 0
    fi
...
# Function to verify if image file matches movie
verify_image_matches_movie() {
    local image_file="$1"
    local movie_title="$2"
    local video_dir="$3"
    ...
    # Only accept images from the same directory as the video file
    # This prevents copying metadata from other movies
    if [ "$image_dir" != "$video_dir" ]; then
        return 1
    fi
...
# Function to find and copy metadata files (fanart, poster, nfo)
find_and_copy_metadata_files() {
    local source_dir="$1"
    local movie_title="$2"
    local destination_dir="$3"
    local final_video_basename="$4"  # Without extension
    local original_video_file="${5:-}"  # Optional: original video file path
    ...
    if [ -n "$original_video_file" ] && [ -f "$original_video_file" ]; then
        ...
        # Video already moved, search for directory
        local video_basename=$(basename "$original_video_file")
        ...
        while IFS= read -r -d '' found_dir; do
            video_parent_dir="$found_dir"
            break
        done < <(find "$source_dir" -type d -iname "*${video_basename}*" -print0 2>/dev/null | head -1)
    fi
    
    # Only search in the video's own directory, not globally
    local search_dirs=()
    [ -n "$video_dir" ] && search_dirs+=("$video_dir")
    [ -n "$video_parent_dir" ] && search_dirs+=("$video_parent_dir")
    
    # If no specific directories found, skip metadata search
    ...
    for search_dir in "${search_dirs[@]}"; do
        ...
        # Find fanart, poster, and nfo files
        while IFS= read -r -d '' meta_file; do
            if [ -f "$meta_file" ]; then
                local file_path=$(realpath "$meta_file" 2>/dev/null || echo "$meta_file")
                
                # Skip if already found
                [ -n "${found_files[$file_path]}" ] && continue
                
                local filename=$(basename "$meta_file")
                local extension="${filename##*.}"
                local should_copy=false
                local new_filename=""
                
                # Verify and process based on file type
                if [[ "$filename" =~ -fanart\.(jpg|jpeg|png)$ ]] || [[ "$filename" =~ fanart\.(jpg|jpeg|png)$ ]]; then
                    # Verify image belongs to this movie
                    if verify_image_matches_movie "$meta_file" "$movie_title" "$search_dir"; then
                        new_filename="${final_video_basename}-fanart.${extension}"
                        should_copy=true
                    fi
                elif [[ "$filename" =~ -poster\.(jpg|jpeg|png)$ ]] || [[ "$filename" =~ poster\.(jpg|jpeg|png)$ ]]; then
                    # Verify image belongs to this movie
                    if verify_image_matches_movie "$meta_file" "$movie_title" "$search_dir"; then
                        new_filename="${final_video_basename}-poster.${extension}"
                        should_copy=true
                    fi
                elif [[ "$filename" =~ \.nfo$ ]]; then
                    # Verify NFO contains "for KODI" and matches movie title
                    if verify_nfo_matches_movie "$meta_file" "$movie_title"; then
                        new_filename="${final_video_basename}.nfo"
                        should_copy=true
                    fi
                fi
                
                # Copy only if verified
                if [ "$should_copy" = true ]; then
                    found_files[$file_path]=1
                    local dest_path="${destination_dir}/${new_filename}"
                    
                    if [ "$DRY_RUN" = true ]; then
                        echo "  [DRY-RUN] Would copy metadata: $(basename "$meta_file") -> $new_filename"
                        ((metadata_count++))
                    elif cp "$meta_file" "$dest_path" 2>/dev/null; then
                        echo "  Copied metadata: $new_filename"
                        ((metadata_count++))
                    else
                        echo "  Failed to copy: $(basename "$meta_file")"
                    fi
                fi
            fi
        done < <(find "$search_dir" -maxdepth 2 -type f \( -iname "*fanart*.jpg" -o -iname "*fanart*.jpeg" -o -iname "*fanart*.png" -o -iname "*poster*.jpg" -o -iname "*poster*.jpeg" -o -iname "*poster*.png" -o -iname "*.nfo" \) -print0 2>/dev/null)
    done
    
    return $metadata_count
}

# Function to remove duplicate copy files
remove_duplicate_copy_files() {
    local directory="$1"
    ...
    # Find all files with _copy_ pattern
    while IFS= read -r -d '' copy_file; do
        if [ ! -f "$copy_file" ]; then
            continue
        fi
        
        local copy_basename=$(basename "$copy_file")
        local copy_dir=$(dirname "$copy_file")
        
        # Extract the original filename by removing _copy_N pattern
        # Pattern: filename_copy_N.ext or filename_copy_1.ext
        if [[ "$copy_basename" =~ ^(.+)_copy_([0-9]+)\.([^.]+)$ ]]; then
            local original_base="${BASH_REMATCH[1]}"
            local copy_num="${BASH_REMATCH[2]}"
            local extension="${BASH_REMATCH[3]}"
            
            local original_file="${copy_dir}/${original_base}.${extension}"
            
            # Check if original file exists
            if [ -f "$original_file" ]; then
                # Get file sizes
                local copy_size=$(stat -f%z "$copy_file" 2>/dev/null || stat -c%s "$copy_file" 2>/dev/null || echo "0")
                local orig_size=$(stat -f%z "$original_file" 2>/dev/null || stat -c%s "$original_file" 2>/dev/null || echo "0")
                
                # If sizes match, it's a duplicate - remove it
                if [ "$copy_size" = "$orig_size" ] && [ "$copy_size" != "0" ]; then
                    if [ "$DRY_RUN" = true ]; then
                        echo "  [DRY-RUN] Would remove duplicate: $(basename "$copy_file")"
                        ((removed_count++))
                    elif rm "$copy_file" 2>/dev/null; then
                        echo "  Removed duplicate: $(basename "$copy_file")"
                        ((removed_count++))
                    fi
                fi
            fi
        fi
    done < <(find "$directory" -type f -name "*_copy_*" -print0 2>/dev/null)
...
rename_subtitle_to_match_video() {
    ...
    local subtitle_base="${subtitle_name%.*}"
    
    # Extract language code from subtitle filename if present
    # Try various patterns: file.eng.srt, file_eng.srt, eng.srt, file-eng.srt
    local lang_code=""
    
    # Pattern 1: .lang.ext or _lang.ext or -lang.ext (most common)
    if [[ "$subtitle_base" =~ [._-](${LANG_PATTERN})$ ]]; then
        lang_code="${BASH_REMATCH[1]}"
    # Pattern 2: Just language code as filename (eng.srt)
    elif [[ "$subtitle_base" =~ ^(${LANG_PATTERN})$ ]]; then
        lang_code="${BASH_REMATCH[1]}"
    # Pattern 3: Language code somewhere in the middle (Title.eng.forced.srt -> extract eng)
    elif [[ "$subtitle_base" =~ [._-](${LANG_PATTERN})[._-] ]]; then
        lang_code="${BASH_REMATCH[1]}"
    fi
    
    # Build new subtitle name: VideoName_lang.ext
    local new_subtitle_name
    if [ -n "$lang_code" ]; then
        new_subtitle_name="${video_basename}_${lang_code}.${subtitle_ext}"
    else
        new_subtitle_name="${video_basename}.${subtitle_ext}"
    fi
...
main() {
    ...
    if [ ! -d "$removed_folder" ]; then
        ...
        if [ "$DRY_RUN" = true ]; then
            echo "[DRY-RUN] Would create: $removed_folder"
        elif ! mkdir -p "$removed_folder" 2>/dev/null; then
    ...
    for item in "$video_folder"/* "$video_folder"/.[^.]*; do
        ...
        if [ "$DRY_RUN" = true ]; then
            echo "  [DRY-RUN] Would move: $item_name -> aA.removed/"
            ((moved_count++))
        elif mv "$item" "$removed_folder/" 2>/dev/null; then
    ...
    # Learn new tag patterns from filenames
    echo ""
    echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
    echo "ANALYZING FILENAMES FOR TAG PATTERNS"
    echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
    learn_tag_patterns "$source_folder"
    echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
    echo ""
    ...
    for file in "${video_files[@]}"; do
        ...
        local series_detected=$?
        
        if [ $series_detected -eq 0 ] && [ -n "$series_info" ]; then
            ...
            if [ "$DRY_RUN" = true ]; then
                echo "[DRY-RUN] Would create: $season_dir"
            elif ! mkdir -p "$season_dir" 2>/dev/null; then
            ...
            if [ "$DRY_RUN" = true ]; then
                echo "[DRY-RUN] Would move: $(basename "$file") -> $final_path"
                # In dry-run, don't actually change file variable
                file="$final_path"
                ...
                while IFS= read -r subtitle_file; do
                    ...
                    if [ "$DRY_RUN" = true ]; then
                        echo "  [DRY-RUN] Would move subtitle"
                        ((subtitle_count++))
                    elif mv "$subtitle_file" "$final_sub_path" 2>/dev/null; then
                        ((subtitle_count++))
                    else
                ...
                # Find and copy metadata files for series episode
                echo "Searching for metadata files..."
                local episode_basename="${clean_series}.S${season_num}E${episode_num}.${clean_episode}"
                find_and_copy_metadata_files "$source_folder" "$series_name" "$season_dir" "$episode_basename" "$original_video_file"
                local metadata_result=$?
                if [ $metadata_result -eq 0 ]; then
                    echo "No metadata files found"
                else
                    echo "Copied $metadata_result metadata file(s)"
                fi
        ...
        if [ -n "$movie_year" ]; then
            ...
            local safe_title
            safe_title=$(echo "$movie_title" | sed 's/[<>:"/\\|?*]/_/g' | sed 's/_*$//' | sed 's/^_*//' | sed 's/ /./g')
            subfolder_name="${safe_title}.${movie_year}"
        else
            local safe_title
            safe_title=$(echo "$movie_title" | sed 's/[<>:"/\\|?*]/_/g' | sed 's/_*$//' | sed 's/^_*//' | sed 's/ /./g')
        ...
        # Create subfolder
        if [ "$DRY_RUN" = true ]; then
            echo "[DRY-RUN] Would create: $subfolder_path"
        elif ! mkdir -p "$subfolder_path" 2>/dev/null; then
        ...
        if [ "$DRY_RUN" = true ]; then
            echo "[DRY-RUN] Would move: $(basename "$file") -> $new_video_path"
            file="$new_video_path"
        elif mv "$file" "$new_video_path" 2>/dev/null; then
        ...
        while IFS= read -r subtitle_file; do
            ...
            if [ "$DRY_RUN" = true ]; then
                echo "  [DRY-RUN] Would move: $subtitle_name -> $(basename "$new_subtitle_path")"
                moved_subtitles+=("$new_subtitle_path")
                ((subtitle_count++))
            elif mv "$subtitle_file" "$new_subtitle_path" 2>/dev/null; then
        ...
        # Find and copy metadata files (fanart, poster, nfo)
        echo "Searching for metadata files (fanart, poster, nfo)..."
        find_and_copy_metadata_files "$source_folder" "$movie_title" "$subfolder_path" "${movie_title}.${movie_year}" "$original_video_file"
        local metadata_result=$?
        if [ $metadata_result -eq 0 ]; then
            echo "No metadata files found"
        else
        ...
        if [ "$has_undefined" = false ]; then
            ...
            if [ "$DRY_RUN" = true ]; then
                echo "[DRY-RUN] Would play video for language identification (skipping in dry-run)"
            else
            ...
            for i in "${!track_languages[@]}"; do
                ...
                local language_code
                if [ "$DRY_RUN" = true ]; then
                    language_code="eng"  # Default for dry-run
                    echo "[DRY-RUN] Would prompt for language selection (using 'eng' as example)"
                else
                ...
                if [ "$DRY_RUN" = true ]; then
                    echo "[DRY-RUN] Would set track $track_num language to: $language_code"
        ...
        if [ -n "$language_string" ]; then
            ...
            if [[ "$current_base" == "${desired_base}_${language_string}" ]]; then
                ...
                if [ "$cleaned_name" != "$current_name" ]; then
                    ...
                    if [ "$DRY_RUN" = true ]; then
                        echo "[DRY-RUN] Would rename: $current_name -> $cleaned_name"
                        file="$new_file_path"
                    elif mv "$file" "$new_file_path" 2>/dev/null; then
                        file="$new_file_path"
                ...
                if [ "$DRY_RUN" = true ]; then
                    echo "[DRY-RUN] Would rename to: $desired_name"
                    file="$new_file_path"
                elif mv "$file" "$new_file_path" 2>/dev/null; then
            ...
            # Rename folder to include language
            if [ -n "$language_string" ]; then
                local new_subfolder_name=""
                if [ -n "$movie_year" ]; then
                    local safe_title
                    safe_title=$(echo "$movie_title" | sed 's/[<>:"/\\|?*]/_/g' | sed 's/_*$//' | sed 's/^_*//' | sed 's/ /./g')
                    new_subfolder_name="${safe_title}.${movie_year}_${language_string}"
                else
                    local safe_title
                    safe_title=$(echo "$movie_title" | sed 's/[<>:"/\\|?*]/_/g' | sed 's/_*$//' | sed 's/^_*//' | sed 's/ /./g')
                    new_subfolder_name="${safe_title}_${language_string}"
                fi
                
                local new_subfolder_path="${output_dir}/${new_subfolder_name}"
                
                # Only rename if folder name is different
                if [ "$subfolder_path" != "$new_subfolder_path" ]; then
                    echo "Renaming folder to include language: $(basename "$new_subfolder_path")"
                    if [ "$DRY_RUN" = true ]; then
                        echo "[DRY-RUN] Would rename folder: $(basename "$subfolder_path") -> $(basename "$new_subfolder_path")"
                    elif mv "$subfolder_path" "$new_subfolder_path" 2>/dev/null; then
                        echo "✓ Folder renamed successfully"
                        # Update all paths
                        subfolder_path="$new_subfolder_path"
                        local video_basename=$(basename "$file")
                        file="${new_subfolder_path}/${video_basename}"
                        # Update subtitle paths
                        for i in "${!moved_subtitles[@]}"; do
                            local sub_basename=$(basename "${moved_subtitles[$i]}")
                            moved_subtitles[$i]="${new_subfolder_path}/${sub_basename}"
                        done
                    else
        ...
        # Remove duplicate copy files
        echo "Checking for duplicate files..."
        remove_duplicate_copy_files "$subfolder_path"
        local dup_removed=$?
        if [ $dup_removed -gt 0 ]; then
            echo "Removed $dup_removed duplicate file(s)"
        else
...
show_help() {
    cat << 'EOF'
Video Organizer and Audio Language Setter v1.6.0
...
EXAMPLES:
  ...
  ./set_v.sh --dry-run /path/to/videos
...
parse_args() {
    case "$1" in
        ...
        --help|-h)
            ...
            ;;
        ""|-*)
            # No arguments or flag, continue to main
            return 0
            ;;
        *)
            # Path argument, continue to main
            return 0
            ;;
    esac
}
